...
if (dbrows($not) == 0) {
	...
	if ($notes['id_user'] != $user['id']) {
		...
		echo '<div class="nav2"><div class="friends_access_list attach_block mt_0 grey">';
		echo group($avtor['id']);
		echo ' <a href="../../user/info.php?id=' . $notes['id_user'] . '"><span style="color:#79358c"><b> ' . $avtor['nick'] . ' </b></span></a> : <a href="list.php?id=' . $notes['id'] . '">';
		echo '<span style="color:#06F;">' . $notes['name'] . '</span></a></div>';
