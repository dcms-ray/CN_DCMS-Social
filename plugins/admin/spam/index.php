...
if (user_access('adm_panel_show')) {
	...
	while ($post = dbassoc($q)) {
		...
		if ($post['razdel'] == 'files_komm') {  // 用户文件
		...
		if ($post['razdel'] == 'notes_komm') {  // 日记
			$notes = dbassoc(dbquery("SELECT * FROM `notes` WHERE `id` = '$post[id_object]' LIMIT 1"));
			echo "<font color='red'>日记</font> | ";
			echo " <a href='../../notes/list.php?id=$notes[id]'>" . htmlspecialchars($notes['name']) . "</a><br />";
		}
		if ($post['razdel'] == 'forum') {  // 论坛主题
		...
		if ($post['razdel'] == 'downnik_komm') {  // 下载
