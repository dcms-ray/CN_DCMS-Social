...
echo '<div class="foot"><img src="/style/icons/str2.gif" alt="*" /> ' . user::nick($ank['id'], 1, 0, 0) . '| <b>书签</b></div>';

if (isset($user) && $ank['id'] == $user['id']) {
	echo '<div class="mess">使用书签功能，您可以保存您感兴趣的人的链接，文件，照片，相册，笔记，讨论<br /></div>';
}
	...
	while ($post = dbassoc($q)) {
		...
		if ($post['type'] == 'forum') {
			...
			if (isset($them['id'])) {
				...
				echo substr(htmlspecialchars($them['text']), 0, 40);
			} else {
				echo '<img src="/style/icons/Forum.gif"> <b>论坛帖子已删除</b>';
			}
			...
			if (isset($notes['id'])) {
				...
				echo substr(htmlspecialchars($notes['msg']), 0, 40);
			} else {
				echo "<img src='/style/icons/diary.gif'> <b>笔记已删除</b>";
			}
			echo " (添加时间 " . vremja($post['time']) . ")";
			...
			if (isset($photo['id'])) {
				...
				echo '<img src="/style/icons/PhotoIcon.gif"> <b>图片已删除</b>';
			}
			...
			if (isset($file_id['id'])) {
				...
				echo '<img src="/style/icons/film.gif"> <b>文件已删除</b>';
			}
			echo " (添加时间 " . vremja($post['time']) . ")";
		}
		echo "</div>";
