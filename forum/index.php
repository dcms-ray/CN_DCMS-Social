...
require_once '../sys/inc/icons.php';
/* 用户封禁 */
	...
	if (isset($user)) {
		...
		if (
			...
			(
				...
				// 编辑帖子的权利（如果帖子中的帖子是灰色的）
			)
		) {
			$msg = $_POST['msg'];
			if (isset($_POST['translit']) && $_POST['translit'] == 1) $msg = translit($msg);
			if (strlen2($msg) < 2) $err[] = '信息过短';
			if (strlen2($msg) > 1024) $err[] = '信息超过1024个字符';
			$mat = antimat($msg);
			if ($mat) $err[] = '消息文本中存在非法字符: ' . $mat;
			...
			if ($set['web'] && is_file(H . 'style/themes/' . $set['set_them'] . '/altername_post_form.php')) {
				require_once H . 'style/themes/' . $set['set_them'] . '/altername_post_form.php';
			} else {
			...
			echo "</form></div>";
			echo "<div class=\"foot\">";
			echo "<img src='/style/icons/str2.gif' alt='*'> <a href=\"/forum/$forum[id]/$razdel[id]/$them[id]/?page=end\" title='返回在帖子'>返回帖子</a><br />";
			...
			echo "</div>";
			echo "<div class=\"foot\">";
			echo "<img src='/style/icons/str2.gif' alt='*'> <a href=\"/forum/$forum[id]/$razdel[id]/$them[id]/?page=end\" title='返回在帖子'>在帖子</a><br />";
			echo "<img src='/style/icons/str2.gif' alt='*'> <a href=\"/forum/$forum[id]/$razdel[id]/\" title='至该组'>" . text($razdel['name']) . "</a><br />";
			echo "<img src='/style/icons/str2.gif' alt='*'> <a href=\"/forum/$forum[id]/\" title='到子论坛'>" . text($forum['name']) . "</a><br />";
			echo "<img src='/style/icons/str2.gif' alt='*'> <a href=\"/forum/\">论坛</a><br />";
			echo "</div>";
			...
			echo "</form></div>";
			echo "<div class=\"foot\">";
			echo "<img src='/style/icons/str2.gif' alt='*'> <a href=\"/forum/$forum[id]/$razdel[id]/$them[id]/?page=end\" title='回到正题'>在帖子</a><br />";
			echo "<img src='/style/icons/str2.gif' alt='*'> <a href=\"/forum/$forum[id]/$razdel[id]/\" title='至该组'>" . text($razdel['name']) . "</a><br />";
			echo "<img src='/style/icons/str2.gif' alt='*'> <a href=\"/forum/$forum[id]/\" title='到子论坛'>" . text($forum['name']) . "</a><br />";
			echo "<img src='/style/icons/str2.gif' alt='*'> <a href=\"/forum/\">论坛</a><br />";
			echo "</div>";
...
if (user_access('forum_for_create') && isset($_GET['act']) && isset($_GET['ok']) && $_GET['act'] == 'new' && isset($_POST['name']) && isset($_POST['opis']) && isset($_POST['pos'])) {
	...
	if (strlen2($name) < 3) $err = '名字太短';
	if (strlen2($name) > 32) $err = '名字太长';
	$opis = $_POST['opis'];
...
echo "</div>";
echo "<div class=\"main\">";
echo "<img src='/style/icons/New.gif'> 新的: <a href='/forum/new_t.php'> 帖子</a> | ";
echo "<a href='/forum/new_p.php'> 回复</a><br />";
if (isset($user)) {
	echo "<img src='/style/icons/top.gif'> 我的: <a href='/user/info/them_p.php?id=" . $user['id'] . "'> 帖子</a> | ";
	echo "<a href='/user/bookmark/forum.php?id=" . $user['id'] . "'>  书签</a> | <a href='/user/info/them_p.php?id=" . $user['id'] . "&komm'> 回复</a><br/>";
	echo "<img src='/style/icons/down.png'> <a href='/forum/files_list.php'>文件列表<br /></a>";}
